...
PHDET <- xtabs(PH ~ Station_ID + Replicate, data = SL22)
PHDet <- as.data.frame.matrix(PHDET)


det.covs <- list(day = Date, pH = PHDet)
str(det.covs)
...
occ.formula <- ~ scale(TDS) + scale(Conductivity)
det.formula <- ~ scale(day) + scale(pH)
